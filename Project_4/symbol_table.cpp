...
bool SymbolTable::add_in_one_entry(string id, string type, string return_type, int belong_scope, int dimension, ParameterList parameter_list, Node* array_type, int local_serial_number)
{
	...
	temp.local_serial_number = local_serial_number;
...
bool SymbolTable::search_entry(string id, const deque<int>& scope_stack) const
{
	for(int i = 0; i < scope_stack.size(); i++)
	{
		if( this->search_list[scope_stack[i]].find(id) != this->search_list[scope_stack[i]].end() )
...
void SymbolTable::printSymbolTable(void) const
{
	...
	cout << setw(10) << "Name" << setw(10) << "Type" << setw(10) << "Scope" << setw(10) << "Dimension" << setw(10) << "Local Serial Number" << endl;
	for(unsigned int i = 0; i < symbol_list_size; i++)
	{
		for(unsigned int j = 0; j < this->symbol_list[i].size(); j++)
		{
			cout.setf(iostream::left);
			cout << setw(10) << this->symbol_list[i][j].id << setw(10) << this->symbol_list[i][j].type << setw(10) << this->symbol_list[i][j].belong_scope << setw(10) << this->symbol_list[i][j].dimension << this->symbol_list[i][j].local_serial_number << endl;
		}
	}
}
...
vector<ENTRY>::const_iterator SymbolTable::get_existing_entry(Node* node, const deque<int>& scope_stack) const
{
	for(unsigned int i = 0; i < scope_stack.size(); i++)
	{
		vector<ENTRY>::const_iterator it = this->symbol_list[scope_stack[i]].begin();
		...
		switch (node_type_number)
		{
			...
			case NODE_IDENTIFIER:
			{
				...
				else
				{
					...
					else if(symbol_table_instance.get_ExistingEntry_DataType(start_node) == "ARRAY")
					{
						if(!array_current_dimesion_check(start_node))
							cout << "[ERROR] Dimension error " << start_node->get_line_no() << endl;
						else
							temp = get_array_basic_DataType(symbol_table_instance.get_existing_entry(start_node)->array_type);
					}
					else
...
{
	cout << "identifier_node " << identifier_node << endl;
	bool result;
	if(!symbol_table_instance.search_entry(identifier_node->get_id()))
	{
		cout << "[ERROR] Undeclared Variable \"" << identifier_node->get_id()  << "\" at line " << identifier_node->get_line_no() << endl;
		result = false;
	}
	if(identifier_node->get_leftmost_child()->get_node_type() == NODE_LAMBDA)
	{
		identifier_node->set_data_type(symbol_table_instance.get_ExistingEntry_DataType(identifier_node));
		result = true;
	}
	else
	{
		if(identifier_node->get_leftmost_child()->get_node_type() == NODE_LPAREN)
		{
			Node* lparen_node = identifier_node->get_leftmost_child();
			ParameterList FunctionProcedure_call_parameter_list;
			TraverseFunctionCallParameter(lparen_node->get_leftmost_child(), FunctionProcedure_call_parameter_list);
			if(symbol_table_instance.get_ExistingEntry_DataType(identifier_node)=="PROCEDURE")
			{
				if(!(FunctionProcedure_call_parameter_list==(symbol_table_instance.get_existing_entry(identifier_node)->parameter_list)))
				{
					cout << "[ERROR] Procedure Parameter not matched at line " << identifier_node->get_line_no() << endl;
					result = false;
				}
				else
				{
					identifier_node->set_data_type(symbol_table_instance.get_existing_entry(identifier_node)->return_type);
					result = true;
				}
			}
			else
			{
				if(!(FunctionProcedure_call_parameter_list==(symbol_table_instance.get_existing_entry(identifier_node)->parameter_list)))
				{
					cout << "[ERROR] Function Parameter not matched at line " << identifier_node->get_line_no() << endl;
					result = false;
				}
				else
				{
		...
		{//Array
			if(symbol_table_instance.get_ExistingEntry_DataType(identifier_node) != "ARRAY")
			{
				cout << "[ERROR] " << identifier_node->get_id() << " at line " << identifier_node->get_line_no() << " is not array type" << endl;
				result = false;
			}
			else if(!array_current_dimesion_check(identifier_node))
			{
				cout << "[ERROR] Dimension error " << identifier_node->get_line_no() << endl;
				result = false;
			}
			else
			{
				identifier_node->set_data_type(get_array_basic_DataType(symbol_table_instance.get_existing_entry(identifier_node)->array_type));
				result = true;
			}
		}
	}
	...
	if(!node->get_is_traversed())
	{
		switch (node->get_node_type())
		{
			...
			case NODE_PROGRAM:
			{
				...
				symbol_table_instance.add_in_one_entry(child->get_id(), "INITIAL", "NONE", scope_stack.front(), 0, ParameterList(), NULL, child->get_local_serial_number());
			...
			case NODE_PROCEDURE:
			{
				...
				symbol_table_instance.add_in_one_entry(node->get_leftmost_child()->get_id(), "PROCEDURE", "NONE", scope_stack.front(), -1, parameter_list, NULL, node->get_local_serial_number());
				node->get_leftmost_child()->set_is_traversed();
				break;
			}
			case NODE_FUNCTION:
			{
				...
				TraverseFunctionDeclareParameter(lparen_node->get_leftmost_child(), parameter_list);
				symbol_table_instance.add_in_one_entry(node->get_leftmost_child()->get_leftmost_child()->get_id(), "FUNCTION", return_type, scope_stack.front(), -1, parameter_list, NULL, node->get_local_serial_number());
				node->get_leftmost_child()->set_is_traversed();
				break;
			}
			case NODE_INT_WORD:
			...
			case NODE_STRING_WORD:
			{
				...
				do
				{
					...
					string s_temp;
					if(! symbol_table_instance.add_in_one_entry(temp->get_id(), get_nodeType_name(type), "NONE", scope_stack.front(), 0, ParameterList(), NULL, temp->get_local_serial_number()) )
			...
			case NODE_ARRAY:
			{
				...
				do
				{
					int type = node->get_node_type();
					string s_temp;
					if(! symbol_table_instance.add_in_one_entry(temp->get_id(), get_nodeType_name(type), "NONE", scope_stack.front(), array_dimension, ParameterList(), node->get_node_array_type(), temp->get_local_serial_number() ) )
						cout << "[ERROR] Redefined variable " << temp->get_id() << " at line " << temp->get_line_no() << endl;
					temp = temp->get_rsibling();
				} while(temp != NULL);
				return;
			...
			{
				/***************************************************************************
				 *At the LHS of ASSIGNMENT Node, it can only be variable or an array entry.*
				 ***************************************************************************/
			...
			case OP_SUB:
			{
				...
				traversal(left_child);//Run the left_child to get the final type
				traversal(right_child);//Run the right_child to get the final type
					...
					if(symbol_table_instance.search_entry(left_child->get_id()))
					{
						...
						else if(left_child->get_leftmost_child() != NULL)
						{
							if(left_child->get_leftmost_child()->get_node_type() != NODE_LPAREN)
							{
								...
								lhs_type = symbol_table_instance.get_existing_entry(left_child)->return_type;
						}
						else
							lhs_type = symbol_table_instance.get_ExistingEntry_DataType(left_child);
					}
					//No Else here since the NODE_IDENTIFIER case will take care of Undeclared problem;
				}
				else
					...
					if(symbol_table_instance.search_entry(right_child->get_id()))
					{
						...
						else if(right_child->get_leftmost_child() != NULL)
						{
							if(right_child->get_leftmost_child()->get_node_type() != NODE_LPAREN)
							{
								...
								rhs_type = symbol_table_instance.get_existing_entry(right_child)->return_type;
						}
						else
							rhs_type = symbol_table_instance.get_ExistingEntry_DataType(right_child);
					}
			...
			case NODE_IDENTIFIER:
			{
				...
				/********************************************
				*Check if the variable is in the list or not*
				*********************************************/
